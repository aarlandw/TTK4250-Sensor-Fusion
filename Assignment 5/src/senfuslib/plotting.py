...
def show_consistency(analysis: ConsistencyAnalysis,
                     axs_nis: Sequence[plt.Axes], fields_nis: Sequence[str],
                     axs_nees: Sequence[plt.Axes], fields_nees: Sequence[str],
                     axs_err: Sequence[plt.Axes], fields_err: Sequence[str],
                     **kwargs):

    def add_stuff(ax, data: ConsistencyData):
        ax.set_yscale('log')
        sym = rf"$\chi^2_{data.dof}$"
        labels = [(f"{sym} {data.alpha:.0%} interval "
                   f"({data.in_interval:.1%} inside)"),
        ...
        for val, label, color in zip(data.thresholds, labels, colors):
            ax.axhline(val, ls='--', label=label, color=color, alpha=0.7)
        ax.legend()
    ...
    for ax, field in zip(axs_err, fields_err):
        err, std = analysis.get_err(field)
        ax.plot(err.times, err.values, label=f'{field} error')
        ax.fill_between(std.times, std.values, -np.array(std.values),
                        alpha=0.5, label=f'$\\pm 1\\sigma_{{{field}{field}}}$')
