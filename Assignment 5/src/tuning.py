...
x0_sim = MultiVarGauss[StateCV](StateCV(0, 0, 6, 0), np.diag([0.1, 0.1, 0.1, 0.1]))

dynamic_model_sim = ModelImm(
    models=[
        ModelCV(std_vel=1e-1),
...
sensor_model_sim = SensorPosClutter(
    sensor=SensorPos(std_pos=1.1), prob_detect=0.7, clutter_density=0.0001
)

sim_imm = Simulator(
    dynamic_model=dynamic_model_sim,
    sensor_model=sensor_model_sim,
    sensor_setter=sensor_setter,
    init_state=MultiVarGauss(x0_sim.mean.with_new_meta(prev_mode=0), x0_sim.cov),
    dt=0.1,
    end_time=80,
    seed="dog",
)
...
dynamic_model_filter = ModelCV(std_vel=0.8)

filter_pdaf = FilterPDA(
    dynamic_model=dynamic_model_filter, sensor_model=sensor_model_filter, gate_prob=0.99
)
